"""
Generate uncertainty quantification (UQ) for uncalibrated Air Temperature (Ta_C).
...
def generate_Ta_C_uncalibrated_UQ(
    ...
    the expected absolute error (uncertainty) of raw/uncalibrated Ta_C estimates using
    ...
    >>> raw_ta_c = np.array([25.3, 26.1, 27.2, ...])
    >>> lower_bound = raw_ta_c - uq
    >>> upper_bound = raw_ta_c + uq
    
    Notes
    -----
    - Model Performance: R² = 0.2660, RMSE = 1.5964, MAE = 1.2074
    ...
    coef_path = Path(__file__).parent / "Ta_C_uncalibrated_UQ_coefficients.csv"
    
    if not coef_path.exists():
        raise FileNotFoundError(
            f"Coefficient file not found: {coef_path}\n"
            "Please ensure Ta_C_uncalibrated_UQ_coefficients.csv is in the JET3 package directory."
        )
