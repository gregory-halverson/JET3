...
def sharpen_soil_moisture_data(
    ...
    if len(coarse_samples) == 0:
        raise ValueError(
            f"unable to retrieve coarse samples for downscaling at {time_UTC} UTC"
        )
    ...
    check_distribution(SM_prediction, "SM_prediction")
    ...
    check_distribution(SM_prediction_coarse, "SM_prediction_coarse")
    SM_bias_coarse = SM_prediction_coarse - SM_coarse
    check_distribution(SM_bias_coarse, "SM_bias_coarse")
    ...
    check_distribution(SM_final_coarse, "SM_final_coarse")
    SM_error_coarse = SM_final_coarse - SM_coarse
    check_distribution(SM_error_coarse, "SM_error_coarse")
    logger.info(
    ...
    check_distribution(SM_error, "SM_error")

    if np.all(np.isnan(SM)):
        raise BlankOutputError(
            f"blank soil moisture output at {time_UTC} UTC"
        )
